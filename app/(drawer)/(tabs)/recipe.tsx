...
const Recipe = () => {
    ...
    return (
        <View style={recipeStyles.container}>
            <Text style={styles.pageTitle}>레시피 등록</Text>
            <View style={styles.headerTop}>
                <Text style={styles.pageSub}>레시피 등록으로 간편한 가게관리</Text>
                <TouchableOpacity onPress={() => { setEdit(m => !m); setSelected([]); }}>
                    <Text style={recipeStyles.editText}>{edit ? '완료' : '편집'}</Text>
                </TouchableOpacity>
            </View>

            <View style={styles.searchBox} pointerEvents="none">
                <Text style={styles.searchPlaceholder}>레시피를 검색하세요.</Text>
            </View>

            <View style={[recipeStyles.card, styles.cardElevated]}>
                <View style={[recipeStyles.tableHeader, styles.headerThin]}>
                    <Text style={[recipeStyles.th, styles.thName]}>레시피 이름</Text>
                    <Text style={[recipeStyles.th, styles.thSmall]}>재료</Text>
                </View>

                {menuList.length === 0 ? (
                    <View style={styles.emptyWrap}>
                        <Text style={styles.emptyText}>저장된 메뉴가 없습니다.</Text>
                    </View>
                ) : (
                    <FlatList
                        data={menuList}
